...
namespace rwe
{
    ...
    bool UnitBehaviorService::handleRepairOrder(UnitId unitId, const RepairOrder& repairOrder)
    {
        auto& unit         = scene->getSimulation().getUnit(unitId);
        auto& repairTarget = scene->getSimulation().getUnit(repairOrder.target);

        if (auto idleState = std::get_if<IdleState>(&unit.behaviourState); idleState != nullptr)
        {
            // request a path to get to the build site

            auto footprint     = unitFactory->getUnitFootprint(repairTarget.unitType);
            auto footprintRect = scene->computeFootprintRegion(repairTarget.position, footprint.x, footprint.y);
            scene->getSimulation().requestPath(unitId);
        ...
        else if (auto movingState = std::get_if<MovingState>(&unit.behaviourState); movingState != nullptr)
        {
            ...
            if (pathToFollow)
            {
                if (followPath(unit, *pathToFollow))
                {
                    ...
                    auto headingAndPitch
                        = computeHeadingAndPitch(unit.rotation, nanoFromPosition, repairTarget.position);
                    auto heading = headingAndPitch.first;
                    auto pitch   = headingAndPitch.second;
                    ...
                    unit.behaviourState = BuildingState{repairOrder.target};
                }
            }
        }
        else if (auto buildingState = std::get_if<BuildingState>(&unit.behaviourState); buildingState != nullptr)
        {
            ...
            {
                // the target does not need to be repaired anymore.
            ...
            if (targetUnit.addRepairProgress(unit.workerTimePerTick))
            {
