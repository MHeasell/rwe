...
namespace rwe
{
    ...
    TEST_CASE("getPieceTransform")
    {
        SECTION("works")
        {
            ...
            REQUIRE(getPieceTransform("foo", modelDef, pieces) == Matrix4x<SimScalar>::identity());
        }
        ...
        SECTION("works when piece has non-zero origin")
        {
            ...
            REQUIRE(getPieceTransform("foo", modelDef, pieces) == expected);
        }
        ...
        SECTION("works with child pieces translated")
        {
            ...
            REQUIRE(getPieceTransform("foo", modelDef, pieces) == fooExpected);

            auto barExpected = Matrix4x<SimScalar>::translation(SimVector(113_ss, 225_ss, 337_ss));
            REQUIRE(getPieceTransform("bar", modelDef, pieces) == barExpected);
        }
        ...
        SECTION("works with child piece rotated")
        {
            ...
            REQUIRE(getPieceTransform("foo", modelDef, pieces) * SimVector(0_ss, 0_ss, 0_ss) == fooExpectedPosition);
