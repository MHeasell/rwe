...
namespace rwe
{
    ...
    public:
        ...
        int toIndex(const GridCoordinates& coords) const
        {
        ...
        void replace(int x, int y, int regionWidth, int regionHeight, const Grid<T>& replacement)
        {
        ...
        {
            assert(x >= 0 && x + g.getWidth() <= this->width);
            assert(y >= 0 && y + g.getHeight() <= this->height);
        ...
        {
            auto x = std::clamp(p.x, 0, width - 1);
            auto y = std::clamp(p.y, 0, height - 1);
        ...
        {
            if (x < 0 || y < 0 || x + replacement.getWidth() > getWidth() || y + replacement.getHeight() > getHeight())
            {
                throw std::logic_error("replacement goes out of bounds");
            }
        ...
        {
            if (x < 0 || y < 0 || x + regionWidth > getWidth() || y + regionHeight > getHeight())
            {
                throw std::logic_error("replacement goes out of bounds");
            }
            if (regionWidth > replacement.getWidth() || regionHeight > replacement.getHeight())
